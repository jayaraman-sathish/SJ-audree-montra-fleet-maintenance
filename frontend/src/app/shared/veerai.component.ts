...
<label>Find vehicle or Job Card<input [(ngModel)]="jobSearch" placeholder="Vehicle number / Job Card" aria-label="Find Veerai job"></label>
<label>Select job<select [(ngModel)]="selectedJobId" (ngModelChange)="changeJob()" aria-label="Veerai job"><option value="">Select a service job</option><option *ngFor="let j of filteredJobs()" [value]="j.id">{{j.vehicle}} · {{j.jobCardNumber}} · {{j.status}}</option></select></label>
<p *ngIf="loadingJobs">Loading service jobs…</p><p *ngIf="jobError" role="alert">{{jobError}} <button (click)="loadJobs()">Retry</button></p><p *ngIf="!loadingJobs&&!jobError&&!filteredJobs().length">No matching jobs found.</p>
</div>
...
<button class="analyse" (click)="analyse()" [disabled]="busy||!activeJobId||!access.trim()||!question.trim()">{{busy?'Analysing evidence…':'Analyse this job'}}</button>
...
export class VeeraiComponent implements OnChanges,OnDestroy {
 ...
 filteredJobs(){const q=this.jobSearch.trim().toLowerCase();return this.jobs.filter(j=>(j.vehicle+' '+j.jobCardNumber).toLowerCase().includes(q));}
