...
@Component({selector:'app-audit',standalone:true,imports:[FleetGridDirective,CommonModule,FleetDateComponent],template:`
<h2>Audit Trail</h2><p role="status">{{message}}</p><div class="card"><div class="fleet-grid-scroll" role="region" aria-label="Records" tabindex="0"><table fleetGrid><thead><tr><th>Occurred On</th><th>User</th><th>Action</th><th>Entity</th><th>Detail</th></tr></thead><tbody><tr *ngFor="let x of rows"><td class="date-cell"><app-fleet-date [value]="x.occurredAt"></app-fleet-date></td><td>{{x.userName}}</td><td>{{x.action}}</td><td>{{x.entityType}}</td><td>{{x.details}}</td></tr></tbody></table></div></div>`})
