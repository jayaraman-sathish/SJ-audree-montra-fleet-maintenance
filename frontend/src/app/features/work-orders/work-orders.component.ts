...
<section class="page">
 ...
 <ng-container *ngFor="let x of rows">
  <tr><td><b>{{x.workOrderNumber}}</b></td><td>{{x.vehicle}}</td><td>{{x.eventType||'Maintenance'}}</td><td class="date-cell"><app-fleet-date [value]="x.openedAt"></app-fleet-date></td><td>{{x.assignedSupervisor||'Unassigned'}}</td><td class="work-status"><span class="state" [class.waiting]="x.status==='Awaiting Assignment'">{{x.status}}</span></td><td><div class="work-actions"><a [routerLink]="['/service-workspace',x.id]">Open Workspace</a><button (click)="toggleDetails(x)" [attr.aria-expanded]="expandedId===x.id" [attr.aria-controls]="'work-details-'+x.id">{{expandedId===x.id?'Hide Details':'Details'}}</button></div></td></tr>
