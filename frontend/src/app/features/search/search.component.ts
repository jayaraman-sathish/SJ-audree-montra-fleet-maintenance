...
@Component({
 ...
 template:`<section class="page">
  ...
  <div class="card" *ngIf="rows.length"><table><tr><th>Type</th><th>Reference</th><th>Result</th><th>Status</th><th></th></tr>
   <tr *ngFor="let x of rows"><td><span class="type">{{x.type}}</span></td><td><b>{{x.key}}</b></td><td>{{x.title}}</td><td>{{x.status}}</td><td><button class="open" (click)="open(x)">Open</button></td></tr>
  </table></div>
 </section>`,
