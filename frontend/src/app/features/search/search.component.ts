 ...
 template:`<section class="page">
  ...
  <article class="record" *ngFor="let x of rows">
   ...
   <ng-container *ngIf="x.type==='Service Visit';else otherRecord">
    <p class="muted">Service opened: {{x.openedAt|date:'dd MMM yyyy, h:mm a'}}</p>
    <section class="issues"><h4>Reported issue / service reason</h4>
     <p *ngIf="!x.issues?.length">{{x.issueSummary}}</p>
     <div class="issue" *ngFor="let issue of x.issues"><strong>{{issue.reference}}</strong><p>{{issue.description}}</p><small>Reported: {{issue.reportedAt|date:'dd MMM yyyy, h:mm a'}}</small></div>
    </section>
    ...
    <section class="job" *ngFor="let job of x.jobs">
     ...
     <div class="metrics">
      <div><span>Primary engineer</span><strong>{{job.engineer}}</strong><small *ngIf="job.taskEngineers?.length">Task engineers: {{job.taskEngineers.join(', ')}}</small></div>
   ...
   <ng-template #otherRecord><p *ngIf="x.reportedAt">Reported: {{x.reportedAt|date:'dd MMM yyyy, h:mm a'}}</p><button (click)="open(x)">{{x.type==='Vehicle'?'View Vehicle 360':'View Requests'}}</button></ng-template>
  </article>
 </section>`,
