...
@Component({selector:'app-job-card',standalone:true,imports:[FleetGridDirective,CommonModule],template:`
...
<div class="card"><h3>Work Items</h3><div class="fleet-grid-scroll" role="region" aria-label="Records" tabindex="0"><table fleetGrid><tr><th>#</th><th>Type</th><th>Description</th><th>SRT</th><th>Checklist</th><th>Status</th></tr><tr *ngFor="let x of rows"><td>{{x.no}}</td><td>{{x.type}}</td><td>{{x.desc}}</td><td>{{x.srt}}</td><td>{{x.check}}</td><td><span class="badge" [ngClass]="x.cls">{{x.status}}</span></td></tr></table></div></div>
