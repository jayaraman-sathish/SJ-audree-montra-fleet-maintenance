...
export class AiConfigurationComponent implements OnInit {
 key=''; updatedBy='AI Administrator'; modules:any[]=[]; apis:any[]=[]; loading=false; saving=false; error=''; message=''; apiEditing=false; apiEditor=false; apiDraft:any={key:'',route:'',method:'GET',module:'',dependenciesText:'',dataSensitivity:'Controlled operational evidence',readOnly:true,owner:'Fleet Platform',version:'v1',enabled:true};
 ...
 startApi(api?:any){this.apiEditor=true;this.apiEditing=!!api;this.apiDraft=api?{...api,_custom:!!api.custom,dependenciesText:(api.dependencies||[]).join(', ')}:{key:'',route:'',method:'GET',module:'',dependenciesText:'',dataSensitivity:'Controlled operational evidence',readOnly:true,owner:'Fleet Platform',version:'v1',enabled:true};} cancelApi(){this.apiEditing=false;this.apiEditor=false;} saveApi(){const d={...this.apiDraft,dependencies:(this.apiDraft.dependenciesText||'').split(',').map((x:string)=>x.trim()).filter(Boolean)};delete d.dependenciesText;const url=d._custom?'/api/ai/api-catalog/'+d.key:'/api/ai/api-catalog';delete d._custom;const request=this.apiEditing?this.http.put(url,d,{headers:this.headers()}):this.http.post(url,d,{headers:this.headers()});request.subscribe({next:()=>{this.message='VeerAI API registration saved.';this.apiEditing=false;this.apiEditor=false;this.loadCatalog()},error:e=>this.error=e.error?.message||'Unable to save API registration'});} 
