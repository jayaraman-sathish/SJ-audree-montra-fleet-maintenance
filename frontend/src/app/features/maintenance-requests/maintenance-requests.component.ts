...
<div class="card"><table><tr><th>Request</th><th>Vehicle</th><th>Source</th><th>Type</th><th>Complaint</th><th>Priority</th><th>Description</th><th>Status</th><th>Action</th></tr>
<tr *ngFor="let x of rows"><td>{{x.requestNumber}}</td><td>{{x.vehicle}}</td><td>{{x.sourceType}}</td><td>{{x.requestType}}</td><td><b>{{categoryName(x.complaintCategoryCode)}}</b><small>{{symptomName(x.symptomCode)}} · {{x.diagnosticTemplateCode}}</small></td><td>{{x.priority}}</td><td>{{x.description}}</td><td>{{x.status}}</td><td class="act"><button *ngIf="x.status==='Open'" class="btn btn-primary" (click)="schedule(x,false)">Schedule Appointment</button><button *ngIf="x.status==='Open'" class="btn btn-outline" (click)="schedule(x,true)">Check In Now</button><button *ngIf="x.status==='Open'" class="btn btn-outline" (click)="setStatus(x,'Cancelled')">Cancel</button><span *ngIf="x.status!=='Open'">{{x.status}}</span></td></tr>
<tr *ngIf="!rows.length"><td colspan="9" class="empty">No maintenance requests.</td></tr></table></div>
