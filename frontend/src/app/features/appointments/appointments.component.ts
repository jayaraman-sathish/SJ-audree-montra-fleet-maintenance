...
<div class="modal-card" *ngIf="startOpen">
 <div class="modal-head"><div><h3>Start Work</h3><p>{{startAppointment?.vehicle}} · {{serviceLabel(startAppointment)}}</p></div><button class="icon-btn" (click)="closeStart()">×</button></div>
 ...
 <div class="source-box"><b>Additional Work</b><span>Select any service requests to include in the same Work Order.</span></div>
...
export class AppointmentsComponent implements OnInit{
 ...
 openStart(x:any){this.startAppointment=x;const related=this.openRequestsForVehicle(x.vehicleId).filter(r=>r.jobCardId==null);this.candidateRequests=related.map(r=>({...r,_selected:(r.sourceType==='Check-In'&&r.sourceReference===x.id)||(x.sourceType==='Maintenance Request'&&r.id===x.sourceReference)}));this.startOpen=true}
 closeStart(){if(!this.saving){this.startOpen=false;this.startAppointment=null;this.candidateRequests=[]}}
 startSelected(){const x=this.startAppointment;if(!x)return;this.saving=true;this.message='';this.h.post<any>(`/api/appointments/${x.id}/start-service`,{}).subscribe({next:r=>{const jobId=r?.jobCard?.id;if(!jobId){this.saving=false;this.message='Work Order was not created.';return}const selected=this.candidateRequests.filter(m=>m._selected);if(!selected.length){this.finishStart(jobId);return}let pending=selected.length;let failed=0;for(const m of selected){this.h.post(`/api/job-cards/${jobId}/work-items`,{workType:'Additional Work',description:`${m.requestNumber} · ${m.description}`,standardRepairHours:null,requiresQc:true,requiresHvAuthorization:false}).subscribe({next:()=>this.h.put(`/api/maintenance-requests/${m.id}/status`,{status:'Converted'}).subscribe({next:()=>{pending--;if(pending===0)this.finishStart(jobId,failed)},error:()=>{failed++;pending--;if(pending===0)this.finishStart(jobId,failed)}}),error:()=>{failed++;pending--;if(pending===0)this.finishStart(jobId,failed)}})}},error:e=>{this.saving=false;this.message=e.error?.message||'Unable to start work'}})}
