...
public static class ControlEndpoints
{
 ...
 public static void MapControlEndpoints(this WebApplication app)
 {
  ...
  app.MapPost("/api/documents/upload",async(HttpRequest request,AppDbContext db)=>{
   ...
   if(!Guid.TryParse(form["vehicleId"].ToString(),out var vehicleId)||!await db.Vehicles.AnyAsync(x=>x.Id==vehicleId))return Invalid("Select a vehicle.");
   ...
   var row=new VehicleDocument{VehicleId=vehicleId,JobCardId=jobId,DocumentType=form["documentType"].ToString(),FileName=Path.GetFileName(file.FileName),Content=bytes,ContentType=pdf?"application/pdf":png?"image/png":"image/jpeg",UploadedBy=user,ExpiresAt=expiry};
